  ...
  return (
    <div className="glass rounded-[40px] p-10 border border-white/10 flex flex-col h-full shadow-2xl relative overflow-hidden bg-gradient-to-b from-slate-900/60 to-transparent">
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          <h2 className="text-3xl font-black uppercase tracking-tighter text-white italic">Ontario Rebate Engine</h2>
          ...
          return (
            <div 
              ...
              className={`group relative rounded-[28px] p-6 border transition-all duration-500 cursor-pointer overflow-hidden ${
                isSelected ? 'bg-emerald-500/10 border-emerald-500/40 shadow-[0_0_30px_rgba(16,185,129,0.1)]' : 'bg-black/40 border-white/5 hover:border-white/20'
              }`}
            >
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${
                      ...
                      {isSelected && <svg className="w-2.5 h-2.5 text-slate-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}><path d="M5 13l4 4L19 7" /></svg>}
                    </div>
                    <span className="text-sm font-black text-white tracking-tight uppercase italic">{lead.location}</span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <span className="text-[8px] bg-black/50 px-2 py-0.5 rounded text-sky-400 font-black uppercase tracking-tighter">Verified AHRI</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xl font-black text-emerald-400 mono leading-none">${lead.rebateAmount.toLocaleString()}</div>
      ...
      <div className="mt-10 pt-8 border-t border-white/5 flex flex-col gap-4">
        <div className="flex justify-between items-center mb-2 px-2">
          ...
          <span className="text-xl font-black text-white mono">${totalValue.toLocaleString()}</span>
        </div>
        <button 
          onClick={handleClaim}
          ...
          className={`w-full py-5 rounded-2xl font-black text-xs uppercase tracking-[0.4em] transition-all relative overflow-hidden group ${
            selectedIds.size > 0 
              ? 'bg-emerald-500 text-slate-950 shadow-[0_20px_40px_rgba(16,185,129,0.3)] hover:scale-[1.02] active:scale-95' 
              : 'bg-slate-800 text-slate-600 cursor-not-allowed'
          }`}
        >
          {selectedIds.size > 0 ? 'Initialize Bulk Filing Protocol' : 'Select Claims for Injection'}
        </button>
      </div>
    </div>
  );
